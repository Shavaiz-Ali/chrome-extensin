...
const Translate = async (text, translatedText) => {
  ...
  try {
    const response = await fetch(
      "http://localhost:5000/api/transltions/update-translations/6735b672d129fcbf3f25dd4e",
      {
        method: "PUT",
        body: {
          text,
    ...
    console.log(error);
    throw new Error("Couldn't translate", error);
  }
};
...
const highlightNode = (node, text) => {
  ...
  const translated_text = translation_input.value;
